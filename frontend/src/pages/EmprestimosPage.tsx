...
export default function EmprestimosPage() {
  ...
  const [form, setForm] = useState({ livro_id: 0 });
  ...
  const handleEmprestar = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      await api.post("/emprestimos/", { livro_id: form.livro_id });
      setShowModal(false);
      setForm({ livro_id: 0 });
      load();
    } catch (err) {
  ...
  const handleDevolver = async (id: number) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Erro ao devolver");
                ...
                <div className="flex items-center gap-3">
                  ...
                  {e.status === "ativo" && (
                    <button
                      ...
                      className="clay-btn clay-btn-primary text-xs py-1.5 px-3 disabled:opacity-60"
                    >
                      {returningId === e.id ? "Devolvendo…" : "Devolver"}
                    </button>
                  )}
                  {e.multa && e.multa > 0 && (
